...
pub async fn listen(
    ...
    loop {
        ...
        if let Some(worker_name) = &inspect {
            ...
            // StdErr::help(&format!("Open devtools://devtools/bundled/inspector.html?experiments=true&v8only=true&ws=localhost:{port}{path} in Chrome", port=DEVTOOLS_PORT, path=socket_url.path()));
